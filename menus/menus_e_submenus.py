...
def encerrar_votacao():
    """
     Gerencia o processo de encerramento da votação.
     Cria a conexão com o banco, chama a função de encerramento
     para validar o mesário e confirmar o encerramento. Se bem-sucedido,
     registra o encerramento no log e redireciona para o menu de resultados.

     Args:
        Nenhum. Os dados são coletados via input durante a execução.

     Returns:
        bool: Retorna True se a votação foi encerrada com sucesso.
              Retorna False se não houver conexão com o banco ou
              se o encerramento falhar.
    """
    conexao, cursor = criar_conexao()
    if not conexao or not cursor:
